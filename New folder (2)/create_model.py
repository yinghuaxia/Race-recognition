...
def CSLYOLOHead(model,heads_len,labels_len,max_boxes_per_cls=10,score_thres=0.001,iou_thres=0.5,nms_type="category_nms"):
    input_ts=model.layers[0].output
    orig_hw=model.layers[0].get_input_shape_at(0)[1:3]
    print(f"score thres: {score_thres}, iou thre: {iou_thres}")
    heads_ts=[]
